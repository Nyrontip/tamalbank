...
| What | Where | Command |
|------|-------|---------|
| Frontend dev server | `frontend/` | `npm run dev` (port 3000) |
...
- **Expense creation uses manual DB transactions** (`$db->begin()` / `commit()` / `rollback()`) — spends deducts from external bank API, then records locally. Both must succeed.
